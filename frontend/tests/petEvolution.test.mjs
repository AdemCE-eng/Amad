...
test('authoritative middle and final stages produce distinct completed, current, and future states', () => {
  const middle = buildEvolutionPresentation({
    stage: 1,
    goalProgress: 50,
    savedAmount: 2500,
    goalAmount: 5000,
  ...
  const final = buildEvolutionPresentation({
    stage: 2,
    goalProgress: 90,
    savedAmount: 4500,
    goalAmount: 5000,
...
test('evolution card labels personal progress and renders actual threshold positions in RTL', async () => {
  const pet = await read('../src/views/PetRoomView.jsx');

  assert.match(pet, /data-testid="pet-evolution-card"/);
  assert.match(pet, /يتطور وفق مدخراتك الشخصية/);
  ...
  assert.match(pet, /right: `\$\{milestone\.at\}%`/);
  assert.match(pet, /className="absolute right-0 top-0/);
  assert.match(pet, /SAR_NUMBER\.format\(evolution\.remainingAmount\)/);
  assert.match(pet, /من مدخراتك الشخصية/);
});
