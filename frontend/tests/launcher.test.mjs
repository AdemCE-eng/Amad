...
test('local launcher enables health-checked ML without blocking fallback startup', async () => {
  ...
  assert.match(launcher, /USE_ML_SERVICE' 'false'/);
  assert.match(launcher, /ML_SERVICE_TIMEOUT_MS' '10000'/);
  ...
  assert.match(launcher, /Starting Nadeem API/);
  assert.doesNotMatch(launcher, /Cheat Controller/);
});
