...
import { Flame, HeartPulse, Sparkles, TrendingUp } from 'lucide-react';
...
export default function CompanionShowcase({ pet, game, emotion, goalProgress, petName, evolution, isSick, isHappy, onTap }) {
  ...
  const nextStageLabel = evolution.nextStage?.name || 'اكتمل التطور';

  return (
    <section
      className="relative mb-3 h-[17.5rem] w-full overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-b from-ink-card to-ink/80 shadow-[0_24px_60px_-38px_rgba(0,0,0,0.95)]"
    ...
    >
      ...
      <svg className="absolute left-1/2 top-1/2 h-[15.5rem] w-[15.5rem] -translate-x-1/2 -translate-y-[53%] -rotate-90" viewBox="0 0 240 240" aria-hidden="true">
      ...
      <MetricChip testId="pet-health-status" icon={HeartPulse} label="الصحة" value={`${pet.health}%`} className={`bottom-11 left-3 ${isSick ? 'border-red-400/30 bg-red-400/10 text-red-300' : 'border-emerald-400/25 bg-emerald-400/10 text-emerald-300'}`} delay={0.2} />

      <button type="button" onClick={handleTap} className="absolute left-1/2 top-1/2 z-10 grid h-52 w-52 -translate-x-1/2 -translate-y-[54%] place-items-center rounded-full focus-visible:outline focus-visible:outline-2 focus-visible:outline-coral" aria-label={`تفاعل مع ${petName}`}>
        <motion.div animate={reacting ? { scale: [1, 1.12, 0.98, 1], rotate: [0, -3, 3, 0] } : { y: [0, -3, 0] }} transition={reacting ? { duration: 0.8, ease: 'easeOut' } : { duration: 3.2, repeat: Infinity, ease: 'easeInOut' }}>
          <Mascot emotion={activeEmotion} stage={game.stage} equipped={game.equipped} size={204} track={!reacting} />
        </motion.div>
      </button>
      ...
      <div className="absolute inset-x-3 bottom-2.5 z-20 flex items-center justify-between gap-2 rounded-2xl border border-white/10 bg-black/15 px-3 py-2 backdrop-blur-md">
        <span className="inline-flex items-center gap-1 text-[9px] font-black text-cream/60"><Sparkles size={11} className="text-violet" /> المس {petName}</span>
        <span className="text-[9px] font-bold text-cream/45">{evolution.currentStage.name}</span>
        <span className="h-px flex-1 bg-gradient-to-l from-coral/50 to-transparent" aria-hidden="true" />
        <span className="text-[9px] font-black text-coral">{nextStageLabel}</span>
      </div>
    </section>
  );
}
