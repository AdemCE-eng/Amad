...
export default function StagedProgress({ steps, activeIndex, testId }) {
    ...
    <div aria-busy="true" data-testid={testId}>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[10px] text-cream/40 font-bold">جاري المعالجة · {progress}%</p>
          <p className="font-black text-sm mt-0.5" aria-live="polite" data-testid={`${testId}-stage`}>{steps[activeIndex]}</p>
        </div>
      </div>
      <div className="h-1.5 bg-white/10 rounded-full overflow-hidden mt-4" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow={progress}>
