...
export default function TransactionRow({ transaction }) {
  ...
  return (
    <article className="p-4 flex justify-between items-center gap-3">
      ...
      <p className={`font-black text-sm whitespace-nowrap ${meta.sign === '+' ? 'text-emerald-400' : 'text-cream'}`}>{meta.sign}{Number(transaction.amount).toFixed(2)} <span className="text-[10px] opacity-60">ر.س</span></p>
    </article>
  );
}
